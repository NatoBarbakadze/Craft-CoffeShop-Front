...
const DetailedIngredientPage = () => {
  ...
  const { response: ingredient, loading, error } = useFetch({
  ...
  if (loading) return <p>Loading ingredient details...</p>;
  if (error) return <p>Error: {error.message}</p>;
  if (!ingredient) return <p>No ingredient found.</p>;
  

  
  const name = ingredient?.data?.name || ingredient?.name || "Unnamed ingredient";
  const description = ingredient?.data?.description || "No description provided";
  const price = ingredient?.data?.price ?? ingredient?.price ?? null;

  return (
    <div>
      <ArrowDiv>
        <img src={arrowImg} alt="arrow image" onClick={() => navigate("/ingredients")} />
        <ArrowTitle>Return to Home</ArrowTitle>
      </ArrowDiv>

      <DetailedPageContent>
        <BigPhoto src={CoffeePhoto} alt="Ingredient" />
        <DetailedPageText>
